...
Almost every API endpoint is authenticated using Bearer JWT tokens. To retrieve an account's JWT token, sign an Otim-formatted SIWE, and call the [Log in](/api-reference/auth/login) endpoint with that SIWE and the signature over it.
